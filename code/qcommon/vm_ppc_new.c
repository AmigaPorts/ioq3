...
#define OP_STACK_MAX_DEPTH	16	// - Cowcat -org 12
...
static void makeFloat(int depth)
{
	...
	if(opStackRegType[depth] == 1)
	{
		...
		{	
			...
			//Inst( "ori", PPC_ORI,	 0, 0, 0 ); // needed ? - Cowcat
...
void VM_Compile( vm_t *vm, vmHeader_t *header )
{
	...
	for ( pass = -1 ; pass < 2 ; pass++ )
	{
		...
		Inst( "lwzx", PPC_LWZX, opStackIntRegisters[opStackDepth-1],
					opStackIntRegisters[opStackDepth-1], R_MEMBASE );	// load from memory base
		...
		assertInteger(opStackDepth-1);
		opStackLoadInstructionAddr[opStackDepth-1] = 0;
		Inst( "lhzx", PPC_LHZX, opStackIntRegisters[opStackDepth-1],
					opStackIntRegisters[opStackDepth-1], R_MEMBASE );	// load from memory base
		opStackRegType[opStackDepth-1] = 1;
		break;
		...
		assertInteger(opStackDepth-1);
		opStackLoadInstructionAddr[opStackDepth-1] = 0;
		Inst( "lbzx", PPC_LBZX, opStackIntRegisters[opStackDepth-1],
					opStackIntRegisters[opStackDepth-1], R_MEMBASE );	// load from memory base
		opStackRegType[opStackDepth-1] = 1;
		break;
...
//		InstImm( "bc", PPC_BC, 4, 0, v );
    ...
    //		Inst( PPC_FRSP, R_TOP, 0, R_TOP );
		...
		{
			SetCache( CACHE_ICACHEINV, vm->codeBase, vm->codeLength	 ); // Hedeon AKA Sonnet lib wizard helped here !! - Cowcat
			//SetCache( CACHE_DCACHEFLUSH, vm->codeBase, vm->codeLength  ); // to test
		}
